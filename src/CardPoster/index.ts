...
const twitter = new Twitter(functions.config().twitter.card_poster)

const factToTweetBody = ({ deck, section, card, creator, topics }: Fact) =>
	`${deck.name} by ${creator.name}${section ? ` (${section.name})` : ''}\n\n${
...
const getTopics = async (deck: Deck) => {
	const topics = await Promise.all(
		deck.topics.map(async topicId => {
			try {
				return new Topic(await firestore.doc(`topics/${topicId}`).get())
			} catch (error) {
...
export const getNextFact = async (deck: Deck): Promise<Fact | null> => {
	...
	try {
		const [section, creator, topics] = await Promise.all([
			card.isUnsectioned ? null : Section.fromId(card.sectionId, deck.id),
...
export const sendNextFact = async () => {
	...
	const promises: Promise<unknown>[] = [deck.updateNextPostedCard()]
	const fact = await getNextFact(deck)

	if (fact) promises.push(sendFact(fact))
