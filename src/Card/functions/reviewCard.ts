	...
	pingable(
		async (
			...
			if (rating === null)
				throw new functions.https.HttpsError(
					'invalid-argument',
					'"rating" must be 0, 1, or 2'
				)

			const now = new Date()
			const { uid } = auth
			const cardRef = firestore.doc(
...
const updateNewCard: UpdateCard = async (
	userData,
	ref,
	now,
	rating,
...
const updateExistingCard: UpdateCard = async (
	userData,
	ref,
	now,
	rating,
	...
	const mastered =
		...
		userData.streak >= Algorithm.MASTERED_STREAK - 1
	const historyRef = ref.collection('history').doc()
	const { e, next } = Algorithm.nextDueDate(rating, userData, now)
	...
	await Promise.all([
		...
		historyRef.set({
			...
			next,
			rating,
			elapsed: userData.last ? now.getTime() - userData.last.date.getTime() : 0,
			viewTime
		})
	])
